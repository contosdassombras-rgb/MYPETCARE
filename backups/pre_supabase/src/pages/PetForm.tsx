  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (existingPet) {
      if (weightVal !== existingPet.weight) {
        ...
        const healthCheck = checkWeightVulnerability(weightVal, existingPet.weight);
        if (healthCheck.alert) {
          if (window.confirm(`${t('significant_weight_change')} (${healthCheck.percentage}%). ${t('is_data_correct')}?`)) {
            alert(t('seek_trusted_vet'));
          }
        }
      }
    ...
    const finalData = { ...formData, weight: weightVal, history: finalHistory };

    if (id) {
      updatePet(id, finalData);
    } else {
      addPet(finalData as Omit<Pet, 'id' | 'events' | 'history'>);
    }
    navigate('/');
  };
