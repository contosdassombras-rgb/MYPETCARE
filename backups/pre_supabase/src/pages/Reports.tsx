...
export const Reports: React.FC = () => {
  ...
  const weightHistoryItems = selectedPet?.history.filter(h => h.type === 'weight') || [];
  const weightBars = weightHistoryItems.length >= 2
    ? weightHistoryItems.slice(-6).map(h => {
        const val = typeof h.value === 'number' ? h.value : parseFloat(h.value as string) || 0;
        return Math.min((val / (selectedPet?.weight || 1)) * 50, 100);
      })
    : [40, 60, 45, 70, 85, 65];
        ...
        <div className="space-y-12">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
            ...
            <Card className="p-10 rounded-[3rem] bg-surface-container-lowest shadow-xl border-none">
              ...
              <div className="h-56 flex items-end gap-4 px-4 pb-4 border-b border-surface-container-high/30">
                {weightBars.map((height, i) => (
                  <div key={i} className="flex-1 bg-primary/5 rounded-t-2xl relative group h-full">
                    <motion.div
                      ...
                      className="absolute bottom-0 w-full bg-primary rounded-t-2xl shadow-lg shadow-primary/20 group-hover:brightness-110 transition-all"
                    />
                  </div>
                ))}
              </div>
